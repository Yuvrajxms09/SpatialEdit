...
class Gemini3pro():
    def __init__(self, api_key_path='keys/secret.env', are_images_encoded=False, model_name="Gemini 3-Pro-Preview"):
    ...
    def prepare_prompt(self, image_links: List = [], text_prompt: str = ""):
        if self.client is not None:
            return self.prepare_prompt_gemini2(image_links, text_prompt)
        prompt_content = []
        text_dict = {
                    "text": text_prompt
                }
        ...
        for image_link in image_links:
            ...
            assert self.use_encode is True
    ...
    def prepare_prompt_gemini2(self, image_links: List = [], text_prompt: str = "", use_encode=True):
        prompt_content = []
        text_dict = {
                    "type": "text",
                    "text": text_prompt
    ...
    def get_parsed_output(self, prompt):
        if self.client is None:
            ...
            try:
                response = response.choices[0].message.content['text']
                return response
            except:
                print("agent response failed")
                return ""
...
if __name__ == "__main__":
    model = Gemini3pro()
    model = Gemini3pro(model_name="Gemini-2.5-flash")
